...
void TcpBaseAlg::rttMeasurementComplete(simtime_t tSent, simtime_t tAcked)
{
    ...
    // send to Tcpemitscalar module for further calculations
    Tcpemitscalar::GetSetInstance(0)->EmitAckReceived();
    Tcpemitscalar::GetSetInstance(0)->RTT_calc(srtt);
    // Hard limit of 400 ms in round trip times for segment latency rate
    if(srtt >0.4)
    {
    Tcpemitscalar::GetSetInstance(0)->Segment_drops();
    }
...
bool TcpBaseAlg::sendData(bool sendCommandInvoked)
{
    ...
    segments_sent = segments_sent +1;
//    emit(segment_countSignal, 1L);
}
